  ...
  return (
    <div className="space-y-12 pb-20">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black mb-2 tracking-tighter uppercase italic text-foreground">Operations <span className="gradient-text italic">Overview</span></h1>
          <p className="text-foreground/40 font-bold tracking-tight">Real-time status of your platform engagements and growth.</p>
        </div>
        
        <div className="flex items-center gap-4">
           <div className="flex items-center gap-6 px-6 py-4 rounded-2xl glass border border-foreground/5">
              <div className="flex items-center gap-2">
                 <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
                 <span className="text-[10px] font-black uppercase tracking-widest text-foreground/40">Network Online</span>
              </div>
              <div className="w-px h-4 bg-foreground/5" />
              <div className="flex items-center gap-2">
                 <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                 <span className="text-[10px] font-black uppercase tracking-widest text-foreground/40">7 Agents Active</span>
              </div>
           </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <motion.div 
            ...
            className="p-5 sm:p-8 lg:p-10 rounded-[28px] sm:rounded-[40px] lg:rounded-[56px] glass border border-foreground/10 flex flex-col gap-6 lg:gap-8 group hover:border-primary/20 transition-all cursor-pointer overflow-hidden relative shadow-huge bg-foreground/5"
          >
            ...
            <div className="flex justify-between items-start relative z-10">
               <div className="w-12 h-12 lg:w-16 lg:h-16 rounded-2xl bg-foreground/5 flex items-center justify-center shadow-xl shadow-white/5 group-hover:scale-110 transition-transform">
                 {stat.icon}
               </div>
            ...
            <div className="relative z-10">
              ...
              <div className="text-[10px] font-black text-foreground/20 uppercase tracking-[0.3em] font-mono">{stat.label}</div>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Recent Activity */}
        <div className="lg:col-span-12 p-6 sm:p-10 md:p-12 rounded-[32px] sm:rounded-[64px] glass border border-foreground/10 shadow-huge bg-foreground/5 relative overflow-hidden h-fit">
          <div className="flex justify-between items-center mb-10">
            <div>
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-foreground/20 mt-1">Live status of current call engagements</p>
            </div>
            <button 
              onClick={() => router.push('/admin/calls')}
              className="px-4 py-2 sm:px-6 sm:py-3 rounded-xl bg-foreground/5 border border-foreground/5 text-[8px] sm:text-[10px] font-black uppercase tracking-widest hover:bg-foreground/10 transition-all"
            >
               View All History
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
            {calls.length > 0 ? calls.map((call: any, i: number) => (
              ...
              >
                ...
                <div className="flex items-center justify-between pt-4 sm:pt-6 border-t border-foreground/5">
                   <div className="flex flex-col gap-1">
                      <div className="text-[10px] font-black text-foreground/20 uppercase tracking-widest flex items-center gap-2">
                         <Clock size={12} />
              ...
              <div className="lg:col-span-3 text-center py-20 bg-foreground/2 border-2 border-dashed border-foreground/5 rounded-[48px]">
                <ShieldCheck size={64} className="mx-auto text-foreground/5 mb-6" />
                <div className="text-sm font-black text-foreground/10 uppercase tracking-[0.4em] italic uppercase">System Operational. No pending tasks.</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-7 p-6 sm:p-8 lg:p-12 rounded-[32px] lg:rounded-[64px] glass border border-foreground/10 shadow-huge bg-foreground/5 h-full relative overflow-hidden">
              ...
              return (
                <div key={i} className="group cursor-pointer">
                  <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-4 text-sm font-black uppercase tracking-[0.2em] text-foreground/80 group-hover:text-foreground transition-colors">
                      {i === 0 ? <Globe size={18} /> : <MapPin size={18} />}
                      {seg.label}
                    </div>
                    <div className="flex items-center gap-4">
                       <span className="text-[10px] text-foreground/20 font-black tracking-widest">{seg.count} CLIENTS</span>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6 lg:gap-8">
           ...
           >
              ...
              <div className="relative z-10">
                 <div className="w-20 h-20 rounded-[32px] bg-foreground/20 flex items-center justify-center mx-auto mb-8 shadow-huge backdrop-blur-xl group-hover:rotate-12 transition-transform">
                    <Tag size={32} className="text-foreground fill-current opacity-80" />
                 </div>
                 <h3 className="text-4xl font-black mb-4 tracking-tighter uppercase italic leading-none">Promotion <span className="text-foreground/40 italic">Manager</span></h3>
                 <p className="text-foreground/70 font-bold tracking-tight text-[10px] mb-8 leading-relaxed max-w-[200px] mx-auto uppercase tracking-widest">Create and manage client engagement campaigns.</p>
           ...
           <motion.div 
             ...
             className="p-6 lg:p-10 rounded-[32px] lg:rounded-[56px] glass border border-foreground/10 shadow-huge bg-foreground/5 flex items-center justify-between group cursor-pointer"
           >
              <div className="flex items-center gap-6">
                 ...
                 <div>
                    ...
                    <div className="text-[9px] font-black text-foreground/20 uppercase tracking-widest mt-1">Full database of registered members</div>
                 </div>
              </div>
              <ChevronRight size={24} className="text-foreground/10 group-hover:text-foreground group-hover:translate-x-1 transition-all" />
           </motion.div>
        </div>
      </div>
    </div>
