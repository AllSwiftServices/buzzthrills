...
export default function AdminCRM() {
  ...
  return (
    <div className="space-y-12">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex flex-wrap gap-4">
           ...
           <select 
             ...
             className="px-6 py-4 rounded-2xl glass border border-border text-[10px] font-black uppercase tracking-widest outline-none cursor-pointer hover:border-primary/40 transition-all appearance-none"
           >
           ...
           <select className="px-6 py-4 rounded-2xl glass border border-border text-xs font-black uppercase tracking-widest outline-none cursor-pointer hover:border-secondary/40 transition-all appearance-none">
      ...
      <div className="rounded-[48px] glass border border-border overflow-hidden shadow-huge bg-background/40 relative">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-border bg-foreground/2">
                <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em] text-foreground/40">Member Details</th>
                <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em] text-foreground/40">Member Status</th>
                <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em] text-foreground/40">Region & Plan</th>
                <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em] text-foreground/40">Joined On</th>
                <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em] text-foreground/40">Management</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border font-bold">
              {profiles.map((profile, i) => (
                ...
                >
                  ...
                  <td className="px-10 py-6 text-sm tabular-nums text-foreground/40">
                    ...
                    <div className="flex gap-2">
                       ...
                       <button 
                         ...
                         className={`p-3.5 rounded-2xl border transition-all hover:scale-105 active:scale-95 ${
                           ...
                           : 'bg-foreground/5 text-foreground/20 border-transparent hover:text-foreground hover:bg-foreground/10'
                         }`}
        ...
        {!loading && profiles.length === 0 && (
          <div className="p-40 text-center flex flex-col items-center justify-center">
             <Users size={80} className="text-foreground/5 mb-8 rotate-12" />
             <div className="text-2xl font-black mb-2 opacity-20">No Members Found.</div>
             <div className="text-[10px] font-bold text-foreground/10 uppercase tracking-widest italic font-mono uppercase">Directory Empty.</div>
          </div>
        )}
      </div>
    </div>
