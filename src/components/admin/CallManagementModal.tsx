...
export default function CallManagementModal({ call, isOpen, onClose, onUpdate, staff }: CallManagementModalProps) {
  ...
  const handleAssign = async (newAssignedTo: string) => {
    ...
    try {
      ...
      // Assigning/unassigning moves status along the lifecycle server-side —
      // sync it here so the lifecycle buttons reflect that without a refetch.
      const data = await res.json();
      if (data.call?.status) setStatus(data.call.status);
      onUpdate();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      >
        <div className="p-5 sm:p-8 border-b border-foreground/5 flex justify-between items-center bg-linear-to-b from-white/5 to-transparent">
          <div>
            <h2 className="text-lg sm:text-2xl font-black tracking-tighter text-foreground">Call Details</h2>
            <p className="text-[8px] sm:text-[10px] font-medium text-foreground/40 tracking-widest mt-1">Review and update this call booking.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 max-h-[80vh] overflow-y-auto">
           ...
           <div className="p-5 sm:p-8 space-y-6 sm:space-y-8 border-b lg:border-r border-foreground/5 bg-foreground/[0.01]">
              <div className="space-y-4">
                <label className="text-[10px] font-black tracking-[0.2em] text-foreground/40">The Recipient</label>
                <div className="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-foreground/5 border border-foreground/10 space-y-3 sm:space-y-4">
                   <div className="flex items-center gap-4">
                      ...
                      <div className="overflow-hidden">
                         <div className="text-[10px] font-black text-foreground/40 tracking-widest">Name</div>
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black tracking-[0.2em] text-foreground/40">Booked By</label>
                <div className="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-foreground/5 border border-foreground/10 flex items-center gap-3 sm:gap-4">
                  ...
                  <div className="overflow-hidden">
                    <div className="text-[10px] font-black text-foreground/40 tracking-widest">Customer</div>
           ...
           <div className="p-5 sm:p-8 space-y-6 sm:space-y-8 border-b border-foreground/5">
              <div className="space-y-4">
                <label className="text-[10px] font-black tracking-[0.2em] text-foreground/40">Call Details</label>
           ...
           <div className="p-5 sm:p-8 space-y-6 sm:space-y-8 border-b lg:border-b-0 lg:border-r border-foreground/5 bg-foreground/[0.01]">
              <div className="space-y-4">
                <label className="text-[10px] font-black tracking-[0.2em] text-foreground/40">Notes to customer</label>
                <textarea 
                  value={adminNotes}
           ...
           <div className="p-5 sm:p-8 pb-32 sm:pb-8 space-y-6 sm:space-y-8 bg-foreground/[0.02]">
              <div className="space-y-4">
                <label className="text-[10px] font-black tracking-[0.2em] text-foreground/40">Status Lifecycle</label>
                <div className="grid grid-cols-1 gap-2">
                  {['pending', 'assigned', 'delivered', 'failed'].map((s) => (
                    <button
                      key={s}
                      onClick={() => { setStatus(s); setError(null); }}
                      className={`py-4 rounded-2xl font-black text-[10px] tracking-widest transition-all border flex items-center justify-between px-6 ${
              ...
              <div className="pt-8 space-y-4 border-t border-foreground/5">
                ...
                {call.status !== 'delivered' && call.status !== 'failed' ? (
                  <button 
                    onClick={handleSave}
                    disabled={uploading}
